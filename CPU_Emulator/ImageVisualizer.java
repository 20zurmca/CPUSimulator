...
public class ImageVisualizer
{
    ...
    private boolean hex; //whether to report in hex or decimal 

 
    /**
     * Constructor for objects of class Viewer
     * @param hex whether the viewer is to report hex or binary instructions
     */
    public ImageVisualizer(boolean hex)
    {
        this.hex = hex;
        try{
            oReader = new BufferedReader(new FileReader("AssemblyCode.o"));
        } catch(Exception e)
        {
            e.printStackTrace();
        }
    ...
    public void processMemory()
    {
        try{
            ...
            int index = 0; //index position to add to memory 
            //adding mememory to an array
            for(int i = 0; i<memory.length()-2; i+=2) 
            {
                String hexbyte = memory.substring(i, i+2);
                mem.addEntry("0x"+hexbyte, index, 1); //add one byte to memeory 
                index++;
            }
        } catch(IOException e)
        {
    ...
    /**
     * Function memoryDisplayHex returns a string formated to display memory from 0x0 in hex
     * Binary or hex depends on the parameters passed to the constructor of the class
     */
    public String memoryDisplayHex()
    {
        int pos = 0;
        String ret = "";
        while(pos < maxmem-4) //process entire memory
        {
    ...
    public String memoryDisplayBin()
    {
        ...
        while(pos < maxmem-4) //process entire memory 
        {
            //get four bytes and interpret it to binary 
            String instruction = mem.getFourBytes(pos);
